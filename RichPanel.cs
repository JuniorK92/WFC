...
/*
 * TODO
 *  Add a expander (collapse/expand)
 *  Check compatibility with scrollable panel
 */
...
namespace WFC
{
    public partial class RichPanel : Panel
    {
         ...
         * Background
         */
        Utils.FillStyle fillStyle = Utils.FillStyle.None;
        [Browsable(true), Category("Background")]
        [DefaultValue(Utils.FillStyle.None)]
        public Utils.FillStyle FillStyle
        {
        ...
        Utils.RoundRectType roundCornerType = Utils.RoundRectType.Upper;
        [Browsable(true), Category("Background")]
        [DefaultValue("Utils.RoundRectType.Upper")]
        public Utils.RoundRectType RoundCornerType
        {
        ...
        Utils.Align headerTextAlign = Utils.Align.Left;
        [Browsable(true), Category("Header")]
        [DefaultValue("Utils.Align.Left")]
        public Utils.Align HeaderTextAlign
        {
        ...
        Utils.Align headerIconAlign = Utils.Align.Right;
        [Browsable(true), Category("Header")]
        public Utils.Align HeaderIconAlign
        {
            ...
            if (this.Width > 1 && this.Height > 1)
            {
                ...
                if (DrawShadow)
                {
                    if (tmpCornerRadius > 0)
                    {
                        ...
                        GraphicsPath graphPathShadow = Utils.GetRoundPath(rectShadow, tmpCornerRadius, this.roundCornerType);
                ...
                GraphicsPath graphPath = Utils.GetRoundPath(rect, tmpCornerRadius, this.roundCornerType);
                if (this.FillStyle == Utils.FillStyle.Gradient)
                {
                ...
                else if (this.FillStyle == Utils.FillStyle.Solid)
                {
        ...
        private void PaintHeader(Graphics graphics)
        {
            ...
            {
                float w = this.Width;
                if (this.headerTextAlign != Utils.Align.Center)
                    w -= tmpShadowOffSet;
                ...
                //Draw the text
                float x = Utils.GetXPos(this.HeaderTextAlign, w, size.Width, margin);
                ...
                if (this.headerTextAlign == Utils.Align.Left)
                    separatorsXs[0] = size.Width + 15;
                else if (this.headerTextAlign == Utils.Align.Center)
                {
                    separatorsXs.Insert(1, x - 10);
                    separatorsXs.Insert(2, x + size.Width + 10);
                }
                else if (this.headerTextAlign == Utils.Align.Right)
            ...
            {
                float w = this.Width;
                if (this.headerIconAlign != Utils.Align.Center)
                    w -= tmpShadowOffSet;

                //Draw the icon
                float x = Utils.GetXPos(this.HeaderIconAlign, w, this.HeaderIcon.Width, margin);
                Point point = new Point((int)x, (headerHeight - this.HeaderIcon.Height) / 2);
                graphics.DrawImage(new Bitmap(this.HeaderIcon), point);

                //Update separator position
                if (this.HeaderIconAlign == Utils.Align.Left)
                    separatorsXs[0] = x + this.headerIcon.Width + 10;
                else if (this.HeaderIconAlign == Utils.Align.Center)
                {
                    separatorsXs.Insert(1, x - 10);
                    separatorsXs.Insert(2, x + this.headerIcon.Width + 10);
                }
                else if (this.HeaderIconAlign == Utils.Align.Right)
        ...
        {
            if (this.Expanded)
                return ResourceWFC.expander_collapse_hor;
            else
                return ResourceWFC.expander_expand_hor;
        }
        ...
        private Rectangle GetExpanderCoordinates(Graphics graphics)
        {
            ...
            int x = (int)Utils.GetXPos(Utils.Align.Right, this.Width - GetShadowOffSet(), bitmap.Width, 5);
